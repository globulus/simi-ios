...
- (JavaLangVoid *)visitYieldStmtWithSMStmt_Yield:(SMStmt_Yield *)stmt {
  if (((SMStmt_Yield *) nil_chk(stmt))->value_ != nil) {
    if (currentFunction_ == JreLoadEnum(SMResolver_FunctionType, INITIALIZER)) {
      [((SMErrorHub *) nil_chk(SMErrorHub_sharedInstance())) errorWithNSString:SMConstants_EXCEPTION_INTERPRETER withSMToken:stmt->keyword_ withNSString:@"Cannot yield a value from an initializer."];
    }
    SMResolver_resolveWithSMExpr_withNSObjectArray_(self, stmt->value_, [IOSObjectArray newArrayWithLength:0 type:NSObject_class_()]);
  }
  return nil;
...
- (JavaLangVoid *)visitAssignExprWithSMExpr_Assign:(SMExpr_Assign *)expr {
  if (!SMResolver_declareWithSMToken_withBoolean_withBoolean_(self, ((SMExpr_Assign *) nil_chk(expr))->name_, true, ((SMToken *) nil_chk(expr->operator__))->type_ == JreLoadEnum(SMTokenType, DOLLAR_EQUAL))) {
    [((SMErrorHub *) nil_chk(SMErrorHub_sharedInstance())) errorWithNSString:SMConstants_EXCEPTION_INTERPRETER withSMToken:expr->name_ withNSString:@"Constant with this name already declared in this scope."];
  }
...
- (JavaLangVoid *)visitVariableExprWithSMExpr_Variable:(SMExpr_Variable *)expr {
  if (![((JavaUtilStack *) nil_chk(scopes_)) isEmpty] && [((id<JavaUtilMap>) nil_chk([scopes_ peek])) getWithId:((SMToken *) nil_chk(((SMExpr_Variable *) nil_chk(expr))->name_))->lexeme_] == JreLoadStatic(JavaLangBoolean, FALSE)) {
    [((SMErrorHub *) nil_chk(SMErrorHub_sharedInstance())) errorWithNSString:SMConstants_EXCEPTION_INTERPRETER withSMToken:((SMExpr_Variable *) nil_chk(expr))->name_ withNSString:@"Cannot read local variable in its own initializer."];
  }
...
+ (const J2ObjcClassInfo *)__metadata {
  ...
  methods[39].selector = @selector(declareWithSMToken:withBoolean:withBoolean:);
